...
| 31 | Multi-format SBOM parsing: Format-detection dispatch layer (`internal/sbom`) with built-in SPDX + CycloneDX parsers (default) and opt-in protobom backend (`USE_PROTOBOM=true`) for maximum format coverage. `.cdx.json` recognized alongside `.spdx.json`. | ✅ Implemented |
